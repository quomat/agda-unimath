...
module _
  ...
  is-property-is-pullback c = {!!}

  is-pullback-Prop : (c : cone f g C) → Prop (l1 ⊔ l2 ⊔ l3 ⊔ l4)
  pr1 (is-pullback-Prop c) = {!!}
```
...
pr1 (exponent-cone T f g c) h = {!!}
pr1 (pr2 (exponent-cone T f g c)) h = {!!}
pr2 (pr2 (exponent-cone T f g c)) h = {!!}
...
inv-gap-cone-Id x y (star , star , p) = {!!}
...
abstract
  is-section-inv-gap-cone-Id :
    ...
    ( gap (const unit A x) (const unit A y) (cone-Id x y) ∘
      inv-gap-cone-Id x y) ~
    id
  is-section-inv-gap-cone-Id x y (star , star , p) = {!!}
...
abstract
  ...
  is-retraction-inv-gap-cone-Id x y p = {!!}
...
pr1 (cone-Id' {A = A} (x , y)) = {!!}
pr1 (pr2 (cone-Id' {A = A} (x , y))) = {!!}
pr2 (pr2 (cone-Id' {A = A} (x , y))) p = {!!}

inv-gap-cone-Id' :
  {l : Level} {A : UU l} (t : A × A) →
  standard-pullback (const unit (A × A) t) (diagonal A) → (pr1 t ＝ pr2 t)
inv-gap-cone-Id' t (star , z , p) = {!!}
...
abstract
  ...
  is-section-inv-gap-cone-Id' .(z , z) (star , z , refl) = {!!}
...
abstract
  ...
  is-retraction-inv-gap-cone-Id' (x , .x) refl = {!!}
...
module _
  ...
  equiv-standard-pullback-htpy :
    standard-pullback f' g' ≃ standard-pullback f g
  pr1 equiv-standard-pullback-htpy = {!!}
```
...
pr1 (refl-htpy-parallel-cone f g c) = {!!}
pr1 (pr2 (refl-htpy-parallel-cone f g c)) = {!!}
pr2 (pr2 (refl-htpy-parallel-cone f g c)) = {!!}
...
pr1 (cone-Π f g c) = {!!}
pr1 (pr2 (cone-Π f g c)) = {!!}
pr2 (pr2 (cone-Π f g c)) = {!!}
...
pr1 (map-standard-pullback-Π f g (α , β , γ) i) = {!!}
pr1 (pr2 (map-standard-pullback-Π f g (α , β , γ) i)) = {!!}
pr2 (pr2 (map-standard-pullback-Π f g (α , β , γ) i)) = {!!}
...
inv-map-standard-pullback-Π :
  ...
  ((i : I) → standard-pullback (f i) (g i)) →
  standard-pullback (map-Π f) (map-Π g)
pr1 (inv-map-standard-pullback-Π f g h) i = {!!}
pr1 (pr2 (inv-map-standard-pullback-Π f g h)) i = {!!}
...
cone-ap :
  ...
  let p = {!!}
pr1 (cone-ap f g (p , q , H) c1 c2) = {!!}
pr1 (pr2 (cone-ap f g (p , q , H) c1 c2)) = {!!}
