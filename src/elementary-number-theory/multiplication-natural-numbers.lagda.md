...
abstract
  ...
  right-zero-law-mul-ℕ zero-ℕ = {!!}

abstract
  right-unit-law-mul-ℕ :
    (x : ℕ) → x *ℕ 1 ＝ x
  right-unit-law-mul-ℕ zero-ℕ = {!!}

  left-unit-law-mul-ℕ :
    (x : ℕ) → 1 *ℕ x ＝ x
  left-unit-law-mul-ℕ zero-ℕ = {!!}
...
abstract
  ...
  right-successor-law-mul-ℕ zero-ℕ y = {!!}

abstract
  commutative-mul-ℕ :
    (x y : ℕ) → x *ℕ y ＝ y *ℕ x
  commutative-mul-ℕ zero-ℕ y = {!!}

abstract
  left-distributive-mul-add-ℕ :
    (x y z : ℕ) → x *ℕ (y +ℕ z) ＝ (x *ℕ y) +ℕ (x *ℕ z)
  left-distributive-mul-add-ℕ zero-ℕ y z = {!!}
...
is-injective-right-mul-succ-ℕ k {zero-ℕ} {zero-ℕ} p = {!!}
...
is-nonzero-left-factor-mul-ℕ .zero-ℕ y H refl = {!!}

is-nonzero-right-factor-mul-ℕ :
  (x y : ℕ) → is-nonzero-ℕ (x *ℕ y) → is-nonzero-ℕ y
is-nonzero-right-factor-mul-ℕ x .zero-ℕ H refl = {!!}
```
...
is-one-right-is-one-mul-ℕ zero-ℕ zero-ℕ p = {!!}
